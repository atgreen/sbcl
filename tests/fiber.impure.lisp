...
(with-test (:name (:fiber :print-fiber-backtrace) :skipped-on :win32)
  (let* ((backtrace-output nil)
         (inspected nil)
         (f1 (make-fiber
              (lambda ()
         ...
         (f2 (make-fiber
              (lambda ()
                ...
                (setf backtrace-output
                      (with-output-to-string (s)
                        (sb-debug:print-fiber-backtrace f1 :stream s)))
                ;; Wake f1 up
                (setf inspected t))
              :name "bt-inspector")))
    (run-fibers (list f1 f2))
    ;; The backtrace output should be a non-empty string
    (assert (plusp (length backtrace-output)))))
